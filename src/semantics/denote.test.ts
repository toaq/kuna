...
test('it denotes anaphora of focused phrases', () => {
	expect(d('Chuq tó jí sá raı, ꝡë cho hụ́to hóa')).toMatchInlineSnapshot(
		"\"∃𝘦. τ(𝘦) ⊆ t0 ∧ AGENT(𝘦)(w) = jí ∧ ruaq.w(λ𝘸. (∀𝘢 : A(𝘢)(jí)(𝘸). ¬∃𝘣 : (∃𝘦'. τ(𝘦') ⊆ t' ∧ raı.𝘸(𝘣)(𝘦')) ∧ ∃𝘦'. τ(𝘦') ⊆ t ∧ cho.𝘸(𝘢, 𝘣)(𝘦'). ∃𝘦'. τ(𝘦') ⊆ t'' ∧ AGENT(𝘦')(𝘸) = 𝘢 ∧ chuq.𝘸(𝘣)(𝘦') | ∃𝘢 : (∃𝘦'. τ(𝘦') ⊆ t' ∧ raı.𝘸(𝘢)(𝘦')) ∧ ∃𝘦'. τ(𝘦') ⊆ t ∧ cho.𝘸(jí, 𝘢)(𝘦'). ∃𝘦'. τ(𝘦') ⊆ t'' ∧ AGENT(𝘦')(𝘸) = jí ∧ chuq.𝘸(𝘢)(𝘦')))(𝘦)\"",
	);
});
